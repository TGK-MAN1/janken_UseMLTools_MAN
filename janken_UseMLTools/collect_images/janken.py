...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), 'Hailo-Application-Code-Examples/runtime/python')))
...
def yolo26n_janken_handler(end_node_data, labels):
    if isinstance(end_node_data, dict):
        raw_output = next(iter(end_node_data.values()))
    else:
        raw_output = end_node_data
...
def custom_visualize_callback(frame, end_node_data, labels):

    detections = yolo26n_janken_handler(end_node_data, labels)
...
def run_inference_pipeline(net, input, batch_size, labels, output_dir,
    ...
    post_process_callback_fn = partial(
        custom_visualize_callback, labels=labels
    )
    ...
    print("MAIN THREAD -> visualize windown opening...")

    visualize(
        output_queue, cap, save_stream_output,
        output_dir, post_process_callback_fn,
        fps_tracker, output_resolution
    )
...
def run_signal_thread_pipeline(args):
    cap = cv2.VideoCapture(0)
    infer_engine = HailoInfer(args.net)
    labels = get_labels(args.labels)

    cv2.namedWindow("janken monitor", cv2.WINDOW_AUTOSIZE)

    print("[finish command]ctrl+c")

    try:
        while True:
            ret, frame = cap.read()
            if not ret:
                print("[-] cannot obtain camera_frame")
                break
        
            inference_results = infer_engine.run_synchronous(frame)

            processed_frame = custom_visualize_callback(frame, inference_results, labels)

            cv2.imshow("janken monitor", processed_frame)
...
def main() -> None:
    ...
    run_inference_pipeline(args.net, args.input, args.batch_size, args.labels,
          ...
          args.output_resolution, args.show_fps, args.framerate)
    """
    run_signal_thread_pipeline(args)
